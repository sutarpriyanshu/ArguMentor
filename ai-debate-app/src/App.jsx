...
const DebateEntry = styled.div`
  ...
  padding: 1rem;
  border-radius: 4px;
  background-color: ${props => props.isUser ? props.theme.userBubbleBg : props.theme.aiBubbleBg};
  color: ${props => props.isUser ? props.theme.userBubbleText : props.theme.aiBubbleText};
  border-left: 4px solid ${props => props.isUser ? props.theme.userBubbleBorder : props.theme.aiBubbleBorder};
`;
...
function App() {
  ...
  const getAIResponse = async (userArgument) => {
    ...
    try {
      ...
      setDebateHistory([...debateHistory, { type: 'user', text: userArgument }, { type: 'ai', text: aiResponse }]);
    } catch (error) {
            ...
            <div className="debate-setup">
              ...
              <form onSubmit={handleStartDebate}>
                <div className="form-group">
                  <label htmlFor="topic">{language === 'en' ? 'Debate Topic:' : 'बहस का विषय:'}</label>
                  <input
                    type="text"
                    id="topic"
                    value={topic}
                    ...
                    required
                    placeholder={language === 'en' ? 'Enter the debate topic' : 'बहस का विषय दर्ज करें'}
                  />
                </div>
                <div className="form-group">
                  <label htmlFor="stance">{language === 'en' ? 'Your Stance:' : 'आपका पक्ष:'}</label>
                  <select
                    id="stance"
                    value={userStance}
                    ...
                    required
                  >
                    <option value="">{language === 'en' ? 'Select your stance' : 'अपना पक्ष चुनें'}</option>
                    <option value="for">{language === 'en' ? 'For' : 'पक्ष में'}</option>
                    <option value="against">{language === 'en' ? 'Against' : 'विरोध में'}</option>
                  </select>
                </div>
              ...
              <DebateHistory>
                {debateHistory.map((entry, index) => (
                  <DebateEntry key={index} isUser={entry.type === 'user'}>
                    <strong>{entry.type === 'ai' ? (language === 'en' ? 'AI' : 'एआई') : (language === 'en' ? 'You' : 'आप')}:</strong>
                    <p>{entry.text}</p>
                    {entry.type === 'ai' && (
                      <button 
                        className="btn-speak" 
